...
# 在 get_dataset 中实现 pathological_modify 选项
def get_dataset(args, **kwargs):
    ...
    if args.iid:
        ...
        if args.pathological_modify:
            print("Using pathological modify sampling")
            user_groups = pathological_modify(
                list(zip(train_dataset.data, train_dataset.targets)),
                args.num_users,
                seed=args.seed
            )
            test_user_groups = pathological_modify(
                list(zip(test_dataset.data, test_dataset.targets)),
                args.num_users,
                seed=args.seed
            )
        elif args.dirichlet:
