...
class LocalUpdate:
    ...
    def update_semi_weights_withoutproto(self, lr, model, global_round, fix_dataset, mix_dataset, additional_feature_banks=None):
        ...
        for iter in range(int(self.args.local_ep)):
            ...
            for batch_idx, (fix_batch, mix_batch) in enumerate(zip(fix_loader, mix_loader)):
                ...
                # print(f"Batch {batch_idx}: Loss = {loss_total.item()}, Loss_u_s = {loss_strong.item()}")
    ...
    def update_semi_weights_ali(self, lr, model, global_round, fix_dataset, mix_dataset, additional_feature_banks=None):
        self.model = model  # 设置 self.model 属性
        self.model.to(self.device)
        self.model.train()
        epoch_loss = []
        ...
        for iter in range(int(self.args.local_ep)):
            ...
            for batch_idx, (fix_batch, mix_batch) in enumerate(zip(fix_loader, mix_loader)):
                ...
                # 提取特征向量
                with torch.no_grad():
                    _,fix_features = self.model(fix_images)
                    _,mix_features = self.model(mix_images)
                # 计算余弦相似度
                cosine_similarity = F.cosine_similarity(fix_features, mix_features, dim=1)
                cosine_similarity_normalized = (cosine_similarity + 1) / 2
                lambda_values = 0.5 + 0.5 * (1 - cosine_similarity_normalized)

                # 计算相似度的均值，得到一个标量
                # lambda_values = torch.clamp(cosine_similarity, 0, 1.0)
                lam = lambda_values.view(-1, 1, 1, 1).to(self.device)
                combined_images = lam * fix_images + (1 - lam) * mix_images

                # 使用 Python 内置的 min 和 max 函数来限制值的范围
                # # 将 fix_images 和 mix_images 加权混合
                # combined_images = lam * fix_images + (1 - lam) * mix_images
                ...
                logits_u_s, _ = self.model(strong)
                loss = criterion_ce(logits_u_s, fix_labels)
                loss = loss.mean()  # 对每个样本的损失求均值，得到标量
                ...
                # 计算总损失
                # loss_total = loss_strong + lam * loss_weak_with_strong + (1 - lam) * loss_weak_with_self
                lam_flat = lam.view(-1)  # 将 lam 变为 [batch_size]
                loss_mixup = lam_flat * loss_weak_with_strong + (1 - lam_flat) * loss_weak_with_self
                loss_mixup = loss_mixup.mean()
                loss_total = loss + loss_mixup
                # test accuracy
                # autual_acc = self.inference(self.model)
                # print(f"|---- Before Batch {batch_idx} -------")
                # print(f"real_acc_for_current_model {autual_acc}")
...
def update_server_weight(args, global_model=None, test_epoch=1, train_dataset=None, test_dataset=None ):
    # global representation, global classifier
    from models import ResNetCifarClassifier
    import torch
    # 设置随机种子
    ...
    optimizer = torch.optim.SGD(
        global_model.parameters(), lr=0.03, momentum=0.9, weight_decay=5e-4
    )
    ...
    with torch.no_grad():
        for inputs, _ in data_loader:
            ...
            if total_softmax is None:
                total_softmax = torch.sum(softmax_outputs, dim=0)
            else:
                total_softmax += torch.sum(softmax_outputs, dim=0)
    return total_softmax / len(data_loader.dataset)
