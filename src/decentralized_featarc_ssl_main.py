...
if __name__ == "__main__":
    ...
    for epoch in tqdm(range(int(args.epochs / args.local_ep))):
        ...
        global_model.save_model(model_output_dir)
    ...
    test_acc_small = np.max(
        [
            small_global_repr_global_classifier(args, global_model, args.finetuning_epoch)
            for global_model in global_models
        ]
    )
    ...
    print("|---- Test Accuracy on small: {:.2f}%".format(100 * test_acc_small))
    print("|---- Test Accuracy on large: {:.2f}%".format(100 * test_acc_large))
