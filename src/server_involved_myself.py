...
if __name__ == "__main__":
    ...
    global_protos = update_global_protos(args, global_model, test_train_dataset, agg_protos_label=global_protos)
    #
    ...
    for idx in idxs_users:
        ...
        global_protos = local_model.update_local_protos(
            # 模型
            model=local_models[idx],
            dataset=train_dataset,
            test_user=user_groups[idx],
            agg_protos_label=global_protos,
        )

        # Aggregate local prototypes using agg_func
    agg_protos = agg_func(global_protos)
    ...
    final_global_protos = agg_protos
    ...
    for epoch in tqdm(range(start_epoch, total_epochs)):
        ...
        #     )
        #     刚开始是model类型
            w, loss, critical_parameter, global_mask, local_mask = local_model.update_semi_weights_myself(
                model=local_models[idx],
                global_round=epoch,
                agg_protos=final_global_protos
            )
        ...
        print("Keys in final_global_protos:", list(final_global_protos.keys()))
        # 聚合前acc
