...
def small_global_repr_global_classifier(args, global_model, test_epoch=60, test_train_dataset=None, test_val_dataset=None):
        ...
        for epoch_idx in tqdm(range(test_epoch)):
            ...
            for batch_idx, (images, labels) in enumerate(test_train_loader):
                ...
                optimizer.zero_grad()
                outputs,_ = global_model_classifer(images)
...
def get_dataset(args,seed=None, **kwargs):
    ...
    if args.iid:
        ...
        if args.dirichlet:
            ...
            user_groups = cifar_noniid_dirichlet(
                args.seed, train_dataset, args.num_users, args.dir_beta, vis=True
            )
            test_user_groups = cifar_noniid_dirichlet(
                args.seed, test_dataset, args.num_users, args.dir_beta
            )
            ...
            print("pathological sampling")
            user_groups = cifar_noniid_pathological(
                args.seed, train_dataset, args.num_users)
            test_user_groups = cifar_noniid_pathological(
                args.seed, test_dataset, args.num_users)
        elif args.pathological_modify:
            print("pathological sampling_modify")
            user_groups = cifar_noniid_pathological_modify(
                args.seed, train_dataset, args.num_users)
            test_user_groups = cifar_noniid_pathological_modify(
                args.seed, test_dataset, args.num_users)
        else:
...
def average_weights(local_weights, local_nums):
    total_num = sum(local_nums)
    global_weights = copy.deepcopy(local_weights[0])
    for key in global_weights.keys():
        # 先将每个权重除以 total_num，再乘以 local_nums[i]
        global_weights[key] = sum((local_weights[i][key] / total_num) * local_nums[i] for i in range(len(local_weights)))
    return global_weights


def average_weights_direct(local_weights):
    """
    Returns the simple average of the weights across clients.
    """
    # 深拷贝第一个客户端的权重，以便初始化全局平均权重
    global_weights = copy.deepcopy(local_weights[0])
